<template>
  <div class="container ticket-page-container">
    <div v-if="!isLoading" class="ticket-page">
      ...
      />
      <BuyCard
        v-if="selectedTimeId && times && $store.getters.getBookingById(selectedTimeId).length > 0"
        :selectedDate="selectedDate"
      ...
      />
      <div v-if="times?.length !== 0 && isShowTime && !isLoadingTime" class="ticket-time-list">
        <el-tooltip
        ...
        >
          <el-button
            :class="selectedTimeId == +item.id && 'ticket-time-list__item_selected'"
            @click="
        ...
        <div v-if="countSelelected() === maxCountSelectedTicket" class="ticket-time-list__max-selected">
      ...
      <div v-else-if="times?.length === 0 && isShowTime && !isLoadingTime" class="ticket-time-not-found">
      ...
      <div v-if="isCategory" class="ticket-category">
        ...
        <div class="ticket-category-item">
          <span class="ticket-category-item__category">Взрослый (РФ и другие страны ЕАЭС) </span>
          <span class="ticket-category-item__price">{{ event.adultPrice }} ₽ </span>
          <div class="ticket-category-item-count">
            <i @click="decreaseAbultCount()" class="el-icon-remove-outline"></i>
            <span class="ticket-category-item-count__number">{{ adultCountTicket }}</span>
            <i @click="increaseAdultCount()" class="el-icon-circle-plus-outline"></i>
          </div>
        </div>
        <div class="ticket-category-item">
          <span class="ticket-category-item__category">Дети в возрасте до 6 лет включительно </span>
          <span class="ticket-category-item__price">{{ event.childPrice }} ₽ </span>
          <div class="ticket-category-item-count">
            <i @click="decreaseChildCount()" class="el-icon-remove-outline"></i>
            <span class="ticket-category-item-count__number">{{ childCountTicket }}</span>
            <i @click="increaseChildCount()" class="el-icon-circle-plus-outline"></i>
          </div>
        </div>
      </div>
    </div>
...
export default Vue.extend({
  ...
  async fetch() {
    ...
    let loop = true
    let startDate = new Date() > new Date(this.event.startDate) ? new Date() : new Date(this.event.startDate)
  ...
  methods: {
    ...
    increaseAdultCount() {
      if (this.adultCountTicket < 5 && this.countSelelected() < this.maxCountSelectedTicket) {
    ...
    increaseChildCount() {
      if (this.childCountTicket < 5 && this.countSelelected() < this.maxCountSelectedTicket) {
            ...
            if (now === date) {
              this.times = res.filter((el) => {
                if (+el.time.split(':')[0] > new Date().getHours()) {
                  ...
                  return (
                    +el.time.split(':')[0] >= new Date().getHours() && +el.time.split(':')[1] > new Date().getMinutes()
                  )
                }
              })
            } else {
